...
FILE_PATTERN = "data/DopplerBroadened*C*.txt"
...
def _legend(ax, **kw):
    ...
    leg = ax.legend(**kw)
    if leg:
        plt.setp(leg.get_frame(), facecolor="#eeeeee", edgecolor="#aaaaaa")
        if leg.get_title():
...
def plot_voltage_vs_temperature(summary, output_dir):
    ...
    ax.errorbar(summary["temps"], summary["mean_voltage"],
                ...
                lw=2, ms=8, markerfacecolor="#111111", markeredgecolor=ACCENT_COLOR,
    ...
    for t, v in zip(summary["temps"], summary["mean_voltage"]):
        ax.annotate(f"{v:.3f} V", (t, v), textcoords="offset points",
                    xytext=(0, 12), ha="center", fontsize=8.5, color="#1a6faf")
    set_dark_style(ax)
...
def plot_peak_pmt_vs_temperature(summary, output_dir):
    ...
    ax.plot(summary["temps"], summary["peak_pmt"], "s-", color="#ff9f7f",
            lw=2, ms=8, markerfacecolor="#111111", markeredgecolor="#ff9f7f")
    for t, p in zip(summary["temps"], summary["peak_pmt"]):
        ax.annotate(f"{int(p)}", (t, p), textcoords="offset points",
                    xytext=(0, 10), ha="center", fontsize=8.5, color="#c04000")
    set_dark_style(ax)
    ...
    for i, (temp, color) in enumerate(zip(temps, colors)):
        ...
        if fit:
            ...
            ax.plot(wlf, gaussian(wlf, *fit["popt"]),
                    color="#111111", lw=1.8, alpha=0.9, label="Gaussian fit")
            ...
            ax.text(0.03, 0.97, info, transform=ax.transAxes,
                    color="#111111", fontsize=8.5, va="top",
                    bbox=dict(facecolor="#eeeeee", alpha=0.75, edgecolor="none"))
        set_dark_style(ax)
...
def plot_fwhm_vs_temperature(fit_results, output_dir):
    ...
    ax.errorbar(temps_fit, fwhms, yerr=fwhm_errs,
                ...
                ecolor="#ff9f7f", markerfacecolor="#111111",
...
def plot_kinetic_vs_setpoint(fit_results, output_dir):
    ...
    ax.plot([lo, hi], [lo, hi], "--", color="#aaaaaa", lw=1.5, label="T_kin = T_set")

    ax.scatter(temps_fit, T_kin_C, color="#1a8a40", s=80, zorder=5,
    ...
    for ts, tk in zip(temps_fit, T_kin_C):
        ax.annotate(f"{tk:.1f}°C", (ts, tk), textcoords="offset points",
                    xytext=(6, 4), fontsize=8.5, color="#1a8a40")
...
def plot_dashboard(dataset, summary, fit_results, output_dir):
    ...
    ax_b.errorbar(summary["temps"], summary["mean_voltage"],
                  yerr=summary["std_voltage"], fmt="o-", color=ACCENT_COLOR,
                  lw=2, ms=6, markerfacecolor="#111111", markeredgecolor=ACCENT_COLOR,
    ...
    ax_c.errorbar(temps_fit, fwhms, yerr=fwhm_errs, fmt="s-", color="#ff9f7f",
                  lw=2, ms=7, markerfacecolor="#111111", markeredgecolor="#ff9f7f",
    ...
    ax_d.plot(summary["temps"], summary["peak_pmt"], "D-", color="#2ca060",
              lw=2, ms=7, markerfacecolor="#111111", markeredgecolor="#2ca060")
    set_dark_style(ax_d)
    ...
    lo, hi = min(temps_fit) - 5, max(temps_fit) + 5
    ax_e.plot([lo, hi], [lo, hi], "--", color="#aaaaaa", lw=1.3, label="1:1")
    ax_e.scatter(temps_fit, T_kin, color="#1a8a40", s=60, zorder=5,
